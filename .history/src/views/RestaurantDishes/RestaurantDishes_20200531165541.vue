...
export default {
  ...
  methods: {
    ...
    async addChoiceMenusPLat(plat) {
      ...
      if (this.idUser != 0) {
        commandStore.commit("addtocommand", plat['@id']);
        ...
        console.log(this.$cookies.get(plat["@id"]));
        commandStore.commit("update");
      }
    },
    async suppChoiceMenusPLat(plat) {
      if (this.$cookies.get(plat["@id"]) == 1 && this.idUser != 0) {
        commandStore.commit("removetocommand", plat['@id']);
      } else {
        this.$cookies.set(
          plat["@id"],
          parseInt(this.$cookies.get(plat["@id"])) - 1
      ...
      commandStore.commit("update");
      for(const value in this.commandUser) {
        console.log(this.commandUser[value]);
      }
