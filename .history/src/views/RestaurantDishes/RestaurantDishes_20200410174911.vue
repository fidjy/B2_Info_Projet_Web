  ...
  <div class="restaurant-container">
    <div class="restaurant-description">
      <v-card class="mx-auto card-margin" width="100%">
        ...
        ></v-img>
        <v-card class="restaurant-presentation animated-title" raised>
          <v-card-title class="restaurant-title text-top"></v-card-title>

          <v-card-title
            class="restaurant-description text-bottom"
          >{{ JSON.parse($route.params.restaurant).description }}</v-card-title>
        </v-card>

        <div class="animated-title">
          <div class="text-top">
            <div>{{ JSON.parse($route.params.restaurant).name }}</div>
          </div>

          <div class="text-bottom">
            <div>{{ JSON.parse($route.params.restaurant).description }}</div>
          </div>
        </div>
