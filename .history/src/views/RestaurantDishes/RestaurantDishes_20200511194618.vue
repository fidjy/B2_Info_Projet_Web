  ...
  <div class="restaurant-container">
    ...
    <div class="menus_restaurant">
      ...
      >
        ...
        <v-card-actions>
          <v-btn @click="addChoiceMenus(plat)" color="primary" outlined>add</v-btn>

          <v-btn @click="suppChoiceMenus(plat)" color="primary" outlined>cancel</v-btn>
        </v-card-actions>
      </v-card>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    axios({
      ...
      .then(response => {
        //console.log(response.data.plats[1]);
        for (let plat in response.data.plats) {
          console.log("response.data.plats : " + response.data.plats[plat]);
        }
