  ...
  <div class="restaurant-container">
    <div class="restaurant-description">
      <v-card class="mx-auto card-margin" width="100%">
        ...
        >
          <v-card class="restaurant-presentation">
            <v-card-title class="restaurant-title">{{ restaurant.name }}</v-card-title>
          </v-card>
        </v-img>
      </v-card>
    </div>

    <div v-for="menu in this.$cookies.keys()" :key="menu.name">{{ $cookies.get(menu) }}</div>
    <!-- 
    {{ choiceMenus }}-->

    <!-- <div v-for="menu in choiceMenus" :key="menu.name">
      {{ menu.menuNumber }}
    </div>-->
...
export default {
  ...
  methods: {
    addChoiceMenus(menu) {
      // if (this.$cookies.keys().includes(menu.name)) {
      //   console.log("yes exist");

      //   for (const element in this.$cookies.keys()) {
      //     if (this.$cookies.get(element) == menu.name) {
      //       this.choiceMenus[element].menuNumber++;
      //       for (const i in this.choiceMenus) {
      //         this.$cookies.set(this.choiceMenus[i].name, this.choiceMenus[i]);
      //       }
      //     }
      //   }
      // } else {
      //   console.log("no");
      //   this.choiceMenus.push(menu);
      //   for (const i in this.choiceMenus) {
      //     this.$cookies.set(this.choiceMenus[i].name, this.choiceMenus[i]);
      //     console.log("create " + this.choiceMenus[i].name);
      //   }
      //   for (const element in this.$cookies.keys()) {
      //     console.log(this.$cookies.get(element));
      //     console.log(menu.name);
      //     if (this.$cookies.get(element) == menu.name) {
      //       console.log("set to 0");
      //       this.choiceMenus[element].menuNumber = 0;
      //     }
      //   }
      // }
      this.choiceMenus.push(menu);
      for (const i in this.$cookies.keys()) {
        this.$cookies.set(i, this.choiceMenus[i]);
      }
    },
    suppChoiceMenus(menu) {
      for (const i in this.$cookies.keys()) {
        // console.log(this.$cookies.get(i).name);
        // console.log(menu.name);
        if (this.$cookies.get(i).name == menu.name) {
          console.log(this.$cookies.get(i));
          this.$cookies.remove(this.choiceMenus[i].name);
        }
      }
    }
  },
  mounted() {
    this.restaurant = JSON.parse(this.$route.params.restaurant);
  }
};
</script>
