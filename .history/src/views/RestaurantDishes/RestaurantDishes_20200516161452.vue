...
export default {
  ...
  methods: {
          ...
          for (const commande in response.data.Commandes) {
            if (response.data.Commandes[commande].state == 0) {
              ...
              for (const plat in response.data.Commandes[commande]["plats"]) {
                ...
                if(this.$cookies.get(response.data.Commandes[commande]["plats"][plat]["@id"]) == undefined){
                  this.$cookies.set(response.data.Commandes[commande]["plats"][plat]["@id"], 0);
                  console.log("cookie name : " + this.$cookies.get(response.data.Commandes[commande]["plats"][plat]["@id"]));
                } else {
                  this.$cookies.set(response.data.Commandes[commande]["plats"][plat]["@id"], parseInt(this.$cookies.get(response.data.Commandes[commande]["plats"][plat]["@id"])) + 1);
                }
                this.userPlats.push(
    ...
    async getCommandStateZero() { console.log("lol"); }
  },
  async mounted() {
    axios({
      method: "get",
