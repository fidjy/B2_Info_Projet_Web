                  ...
                  <v-item class="center-picture-resto" v-slot:default="{ active, toggle }">
                    <div class="router-link-sing-in">
                      ...
                      >
                        ...
                        <div v-if="checkFavorite(item.usersFavorits)">
                          <v-btn
...
export default {
  ...
  methods: {
    ...
    async suppFavorit(restaurantId) {
      ...
      for (let value in this.user.favoritRestaurants) {
        if (this.user[value].favoritRestaurants.id != restaurantId) {
          favorit.push(this.user[value].favoritRestaurants);
        }
      }
    ...
    async addFavorit(restaurantId) {
      await axios
        ...
        .then(response => {
          this.usersFavoriteList = response.data.usersFavorite;
        })
        .catch(function(error) {
          console.log(error);
        });
      ...
      for (let value in this.user) {
        if (this.user[value].id != restaurantId) {
          favorit.push(this.user[value]);
        }
      }
