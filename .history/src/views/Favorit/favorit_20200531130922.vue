...
export default {
  ...
  methods: {
    ...
    async getRestaurants() {
      this.items = [];
      await axios.get("/api/users/" + this.idUser).then(response => {
        this.user = response.data;
        for (const value in response.data.favoritRestaurants) {
          this.items.push(response.data.favoritRestaurants[value]);
        }
      });
    }
  },
...
<style>
@import url("Assets/favorit.scss");
</style>
