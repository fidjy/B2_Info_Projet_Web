...
export default {
  ...
  watch: {
    ...
    if (this.$cookies.get("log") && this.$cookies.get('addressDelivery') != undefined) {
     // console.log(this.$cookies.get('addressDelivery'));
      await axios
        .get("/api/users/" + this.$cookies.get("idUser"))
        .then(response => async function() {
          let initializeCommand = true;
          ...
          for (const commande in response.data.Commandes) {
            ...
            if (response.data.Commandes[commande].state == 0) {
              this.commandeNotFinishedId = response.data.Commandes[commande].id;
              initializeCommand = false;
            }
          }
        console.log("initialsie" + initializeCommand);
        ...
        if (initializeCommand) {
          ...
          .catch(function(error) {
            ...
            console.log("/api/commandes/" + this.commandeNotFinishedId);
            console.log(this.$cookies.get('addressDelivery'));
            await axios.put("/api/commandes/" + this.commandeNotFinishedId, {
              address: this.$cookies.get('addressDelivery')
            })
            .then(response => {
              console.log(response);
