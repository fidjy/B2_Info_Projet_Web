  ...
  <v-content>
    <v-card class="overflow-hidden height-nav-bar">
      ...
      >
        ...
        <div class="sign-in">
          <router-link class="router-link-sing-in" :to="{ name: 'Login', params: { propsAdresses: JSON.stringify(streets) } }">
            <a>Login</a>
            {{ streets }}
          </router-link>
        </div>
...
export default {
  ...
  computed: {
    streets: function (){
      if(this.propositionAdresse != ""){


        this.propositionAdresse.forEach(element => console.log(element));

this.propositionAdresse.forEach(function(item){
  item
});


        return this.propositionAdresse;
      }
      return "this.streets"
    }
  }
