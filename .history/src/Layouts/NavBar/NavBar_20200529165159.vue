...
export default {
  ...
  methods: {
    ...
    logout() {
      ...
      store.commit("setUserToNull");
      store.commit("setRoleNull");
  ...
  computed: {
    command() {
      return store.state.commandBox;
    },
    idUser() {
      return store.state.idUser;
    },
    log: function() {
      this.getUserInformations();
    ...
    if (this.$cookies.get("log") == "true") {
      await axios
        ...
        .then(response => {
          ...
          store.commit("getUserRoles", response.data.data.roles);
          this.$cookies.set("log", true);
        })
        .catch(function(error) {
          console.log(error);
