    ...
    <v-sheet class="overflow-hidden height-nav-bar">
      ...
      >
        <div class="font-italic margin-cart">
          <p>Your order</p>
          <v-card v-for="command in commandBox" :key="command.name" class="mx-6" max-width="544">
            {{ command }}
            <div v-if="command.state = 0">
              <v-card-text>
  ...
  mounted: function() {
    axios
      ...
      .then(response => {
        ...
        for (let value in response.data.Commandes) {
          this.commandBox.push(response.data.Commandes[value]);
          console.log(this.commandBox[value]['state']);
        }
      })
      .catch(function(error) {
        console.log(error);
