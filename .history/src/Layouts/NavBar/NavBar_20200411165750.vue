<template>
  <v-content>
    <v-sheet class="overflow-hidden height-nav-bar">
      ...
      <v-navigation-drawer v-model="drawerCart" absolute temporary :right="position" width="344">
        <div v-for="menu in this.$cookies.keys()" :key="menu.name" class="font-italic margin-cart">
          <p>Your order</p>
          <v-card class="mx-auto" max-width="544">
            <v-card-text>
              <div>Word of the Day</div>
              <p class="display-1 text--primary">be•nev•o•lent</p>
              <p>{{ $cookies.get(menu).name }}</p>
            </v-card-text>
            <v-card-actions>
              <div class="my-2">
        ...
        <v-list nav dense>
          <v-list-item-group active-class="deep-purple--text text--accent-4">
            ...
            <v-list-item>
              ...
              <v-list-item-title>
                <router-link class="router-link-sing-in" :to="{ name: 'BugReport' }">Bug report</router-link>
              </v-list-item-title>
            </v-list-item>
