...
export default {
  ...
  watch: {
    ...
    detect: function() {
      while (this.$cookies.get("updatePlats") == true) {
        this.commandBox = [];
        axios
          .get("/api/users/" + this.$cookies.get("idUser"))
          .then(response => {
  ...
  mounted: async function() {
    let commandInititalize = true;

    //check if there a command in state 0
    await axios
      .get("/api/users/" + this.$cookies.get("idUser"))
      .then(response => {
        for (const progressCommand in response.data.Commandes) {
    ...
    // if there is no command we create one
    if (commandInititalize  == true) {
      await axios
        .get("/api/users/" + this.$cookies.get("idUser"))
        .then(response => {
          this.commands = [];
          for (const commande in response.data.Commandes) {
            this.commands.push(response.data.Commandes[commande]);
          }
        })
        .catch(function(error) {
          console.log(error);
      ...
      await axios
        .post("/api/users/" + this.$cookies.get("idUser"))
        .then(response => {
          this.commands = [];
          for (const commande in response.data.Commandes) {
            this.commands.push(response.data.Commandes[commande]);
