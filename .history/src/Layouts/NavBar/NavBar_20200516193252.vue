  ...
  <v-content>
    <v-sheet class="overflow-hidden height-nav-bar">
      ...
      <v-navigation-drawer
        ...
        height="400"
      >
      {{ $cookies.get(updatePlats) }}
        <div v-if="$cookies.get(updatePlats) == false">{{ $cookies.set('updatePlats', true) }}</div>
        <div v-if="$cookies.get(updatePlats) == 'true'" class="font-italic margin-cart">
          <p>Your order</p>
          <v-card
...
export default {
  ...
  mounted: function() {
    this.$cookies.set('updatePlats', true);
    axios
      .get("/api/users/" + this.$cookies.get("idUser"))
      .then(response => {
        for (const progressCommand in response.data.Commandes) {
