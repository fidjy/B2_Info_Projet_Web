        ...
        <v-list nav dense>
          <v-list-item-group active-class="deep-purple--text text--accent-4">
            <router-link :to="{ path: searchIconsRoles($cookies.get('actualRole'))[1] }">
              {{searchIconsRoles(this.$cookies.get('actualRole'))[1]}}
              <v-list-item>
                <v-list-item-icon>
          ...
          <v-card flat class="py-12">
            <v-card-text>
              <v-row align="center" justify="center">
                <v-col v-if="this.$cookies.get('log')" cols="12">
                  <div class="text-center">User Type</div>
                </v-col>
                <v-btn-toggle v-if="this.$cookies.get('log')" v-model="toggleExclusive" mandatory>
                  <router-link
                    v-for="item in roles"
                    :key="item.id"
